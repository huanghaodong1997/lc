...
class Solution:
    def minRefuelStops(self, target: int, startFuel: int, stations: List[List[int]]) -> int:
        ...
        ans = -1
        step = 0
        for loc, gain in stations:
            if not h or (pos + fuel) >= loc:
                heapq.heappush(h, (target - gain, loc, gain))
            elif (pos + fuel) < loc:
                while h and (pos + fuel) < loc:
                    _, station, add_fuel = heapq.heappop(h)
                    #if station < pos: continue
                    if pos + fuel < station: break
                    new_fuel = fuel - (station - pos) + add_fuel
                    pos = station
                    fuel = new_fuel
                    step += 1
                if pos + fuel >= target: return step
                if (pos + fuel < loc): return -1
                heapq.heappush(h, (target - gain, loc, gain))
        if h:
            while h and (pos + fuel) < target:
                _, station, add_fuel = heapq.heappop(h)
                if (pos + fuel) < station: break
                new_fuel = fuel - (station - pos) + add_fuel
                pos = station
